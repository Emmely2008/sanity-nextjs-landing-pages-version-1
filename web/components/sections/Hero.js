...
function Hero(props) {
  ...
  var isTouch; // = global.window && "ontouchstart" in global.window;
  var isSafari; // = !!global.navigator.userAgent.match(/Version\/[\d\.]+.*Safari/);
  React.useEffect(() => {
    isTouch = global.window && "ontouchstart" in global.window;
    isSafari = global.navigator && !!global.navigator.userAgent.match(/Version\/[\d\.]+.*Safari/);
    const canvas =
      heroType == "three" &&
      new Canvas({
  ...
  if (heroType == "three") {
    return (
      <>
        <h1 className={styles.three_title}>{heading}</h1>
        <div className={styles.three_tagline}>{tagline && <SimpleBlockContent blocks={tagline} />}</div>
        <div id="container">
